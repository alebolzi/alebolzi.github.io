...
const CONFIG = {
  ...
  pac:     { bufferPct: 0.10, arrotondaA: 10, sogliaAnni: 5 },
...
function calculatePACPlan(input, gap, reb, tgt){
  ...
  const verdict = mesi==null ? 'nessuno' : (mesi===0?'già-nei-range':(anni>CONFIG.pac.sogliaAnni?'B':'A'));
  return {pac, gaps, gapTot, mesi, anni, verdict,
    taxCost:reb.costoVendite, taxUnknown:reb.taxUnknown,
    venditeN:reb.vendite.length, venditeEur:sum(reb.vendite.map(v=>v.amount)),
    soglia:CONFIG.pac.sogliaAnni};
}
...
function analyze(input){
  ...
  rctx.trace.push({g:'Piano PAC vs redistribuzione immediata', items:[
    ...
    {f:'Costo fiscale della redistribuzione immediata', v: pacPlan.taxUnknown?'da verificare (plusvalenze senza costo di acquisto)':fmtE(pacPlan.taxCost)},
    {f:'Regola: PAC oltre '+pacPlan.soglia+' anni → redistribuzione immediata', v:'verdetto: '+pacPlan.verdict},
  ]});
  const ess=Math.max(1,input.spese.essenziali);
...
function updateAgeLive(){
  const input=$('#f-eta'); if(!input) return;
  const value=clamp(+input.value,18,90);
  ...
  input.style.background='linear-gradient(to right, '+stage.color+' 0%, '+stage.color+' '+pct+'%, rgba(27,25,18,.16) '+pct+'%, rgba(27,25,18,.16) 100%)';
  ...
  const display=input.closest('.age-field');
  if(display){
    display.style.setProperty('--age-color',stage.color);
    qsa('.age-icon i, .age-icon svg',display).forEach((icon,i)=>{
...
function renderResults(R){
  ...
  const vTxt={
    ...
    'A':'Il PAC di '+fmtE(PP.pac)+'/mese colma il divario in ~'+fmtDur(PP.mesi)+' senza vendere nulla: zero imposte, zero costi di transazione. Essere sotto la soglia dei '+PP.soglia+' anni rende il PAC la strada migliore.',
    'B':'Con il solo PAC servirebbero ~'+fmtDur(PP.mesi)+' per rientrare nei target: oltre la soglia dei '+PP.soglia+' anni il tempo perso (capitale mal allocato) supera in media il risparmio fiscale. Conviene la redistribuzione immediata — oppure un approccio misto: immediata sulla liquidità in eccesso, PAC per il resto.',
  ...
  let pacHTML='<div class="stat-row"><span class="sk">Capacità di risparmio stimata (reddito − spese)</span><span class="sv">'+fmtE(Math.max(0,ps.capacity))+'/mese</span></div>'
    ...
    +'<div class="duo" style="gap:16px;margin:22px 0 18px">'
      ...
      +'<div class="planbox'+(vB?' win':'')+'">'
        ...
        +'<div class="stat-row"><span class="sk">Costo stimato (tasse + commissioni)</span><span class="sv">'+(PP.taxUnknown?'<b style="color:var(--warn)">da verificare</b>':fmtE(PP.taxCost))+'</span></div>'
        +'<p class="hint" style="margin-top:10px">Vendere realizza le plusvalenze maturate, tassate al 26%: è il prezzo della velocità. I costi sono stimati; senza costo di acquisto indicato, l\u2019impatto fiscale non è calcolabile.</p>'
      +'</div>'
    +'</div>'
    ...
    +'<p class="hint" style="margin-top:18px"><b>Nota fiscale.</b> Il Piano A è quasi sempre più efficiente fiscalmente: ogni versamento non genera tasse, ogni vendita sì (26% sulle plusvalenze, in Italia). La soglia dei '+PP.soglia+' anni è il punto in cui, in media, il costo-opportunità del capitale mal allocato supera il risparmio fiscale — parametro modificabile in CONFIG.pac.sogliaAnni.</p>';
